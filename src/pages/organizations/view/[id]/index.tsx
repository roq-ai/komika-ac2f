...
import { deleteMangaById } from 'apiSdk/manga';
...
function OrganizationViewPage() {
  ...
  const mangaHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('manga', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Manga
                    </Text>
                    <NextLink passHref href={`/manga/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.manga?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/manga/view/${record.id}`)} key={record.id}>
                            <Td>{record.title}</Td>
                            <Td>
